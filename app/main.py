...
def login(request: Request, login_data: LoginRequest, db: Session = Depends(get_db)):
    """Login and receive JWT token (Rate limited: 5 attempts per minute)"""
...
@app.get("/customer/dashboard", tags=["Customer"])
def customer_dashboard(
    current_user: models.User = Depends(get_current_user),
    db: Session = Depends(get_db)
):
    """Customer: Personal dashboard with account summary"""
    ...
    recent_transactions = db.query(models.Transaction).filter(
        (models.Transaction.from_account_id.in_(all_account_ids)) |
        (models.Transaction.to_account_id.in_(all_account_ids))
    ...
    return {
        ...
        "loans": {
            ...
            "active_loans": len([l for l in loans if l.status == models.LoanStatus.APPROVED]),
            "pending_loans": len([l for l in loans if l.status == models.LoanStatus.PENDING]),
            "loans": [
                {
                    "id": l.id,
...
@app.get("/auditor/logs", tags=["Auditor"])
def get_audit_logs(
    limit: int = 100,
    current_auditor: models.User = Depends(get_current_auditor),
    db: Session = Depends(get_db)
):
